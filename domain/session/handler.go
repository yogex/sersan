...
func (h SessionHandler) Create(w http.ResponseWriter, r *http.Request) {
    ...
    for ; ; i++ {
        ...
        defer done()
        log.Printf("Session attempted - %s - %d", startedGrid.URL.Hostname(), i)
        ...
        case <-ctx.Done():
            ...
            case context.DeadlineExceeded:
                log.Printf("Session attempted timeout %d", conf.NewSessionAttemptTimeout)
                ...
                log.Printf("Session failed - %s - %s", startedGrid.URL.Hostname(), err)
...
func (h SessionHandler) Proxy(w http.ResponseWriter, r *http.Request) {
    ...
    go func(w http.ResponseWriter, r *http.Request) {
        ...
        if r.Method == http.MethodDelete && len(fragments) == 3 {
            defer func() {
                err := h.SessionService.Delete(sessionInfo.PodName)
                if err != nil {
                    log.Printf("Unable to delete pod %s", sessionInfo.PodName)
                }
            }()
        }
    }(w, r)
